...
public class TaxModel {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(test1);
        System.out.println(String.format("Tax: %.2f", appliedTax));
        System.out.println(String.format("Total Cost: %.2f", totalCost));

    }

    // Calculates Tax based on tax type and price
    public static double applyTax(int taxType, double price){
        return (price * TAXES[taxType]);
    }

    // returns the corresponding Tax Type from a string, -1 if type not found
    // None -> 0, GST -> 1, HST -> 2
